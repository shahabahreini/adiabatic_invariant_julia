...
def Plotter(save_filename):
    ...
    for fname in filelst:
        ...
        df = pd.DataFrame(data, columns=["timestamp", "value4", "value6"])
        df.rename(columns={"value4": "X", "value6": "Z"}, inplace=True)
        ...
        if is_multi_files:
            ...
            kappa = parameter_dict["kappa"]
        plt.plot(x_, y_, label=f"$\kappa = {kappa}$")

    if do_plot_line_from_origin:
        xp_, yp_ = point_finder(x_.iloc[0], y_.iloc[0], x_.iloc[-1])
        plt.plot(xp_, yp_, "--", label=f"line passes origin")

    # plt.ylabel(axis_to_select)
    # plt.xlabel(r"$\tau$")

    plt.rcParams["figure.dpi"] = 150
    plt.ylabel(r"$\widetilde{Z}$")
    plt.xlabel(r"$\tau$")
    plt.suptitle(r"$\widetilde{Z}$ vs $\tau$", fontsize=12)
    plt.title(
